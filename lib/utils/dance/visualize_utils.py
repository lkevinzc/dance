...
def visualize_snake_evolution(img, data):
    img = img_utils.bgr_to_rgb(img)
    plt.imshow(img)
    for poly, gt in zip(data['i_it_py'], data['i_gt_py']):
        poly = poly * 4
        gt = gt * 4

        poly = np.append(poly, [poly[0]], axis=0)
        gt = np.append(gt, [gt[0]], axis=0)

        plt.plot(poly[:, 0], poly[:, 1])
        plt.scatter(poly[0, 0], poly[0, 1], edgecolors='w', alpha=0.5)
        ...
        for i, pt in enumerate(poly):
            ...
            plt.arrow(x, y, dx, dy, lw=0.5, alpha=0.5)

    plt.show()


def visualize_dance_evolution(img, data):
    img = img_utils.bgr_to_rgb(img)
    plt.imshow(img)
    for poly, gt in zip(data['init_box'], data['targ_poly']):
        poly = poly * 4
        gt = gt * 4
...
def visualize_dance_edge(edge_map):
    edge_map[edge_map == 255] = 128
