...
def prepare_training_init(ret, batch):
    ...
    init.update({
        'ind':
        torch.cat([torch.full([ct_num[i]], i) for i in range(ct_01.size(0))],
                  dim=0)
    })
...
def get_box_match_ind(pred_box, score, gt_poly):
    ...
    gt_box = torch.cat(
        [torch.min(gt_poly, dim=1)[0],
         torch.max(gt_poly, dim=1)[0]], dim=1)
    iou_matrix = data_utils.box_iou(pred_box, gt_box)
    iou, gt_ind = iou_matrix.max(dim=1)
    box_ind = ((iou > snake_config.box_iou) *
               (score > snake_config.confidence)).nonzero().view(-1)
...
def prepare_training(ret, batch):
    ct_01 = batch['ct_01'].byte()
    init = {}
    init.update({'init_box': collect_training(batch['init_box'], ct_01)})
    init.update({'targ_poly': collect_training(batch['targ_poly'], ct_01)})
...
def get_gcn_feature(cnn_feature, img_poly, ind, h, w):
    ...
    gcn_feature = torch.zeros(
        [img_poly.size(0),
         cnn_feature.size(1),
         img_poly.size(1)]).to(img_poly.device)
    for i in range(batch_size):
        poly = img_poly[ind == i].unsqueeze(0)
        feature = torch.nn.functional.grid_sample(cnn_feature[i:i + 1],
                                                  poly)[0].permute(1, 0, 2)
...
def get_pconv_ind(n_adj, n_nodes, device):
    ...
    outer_ind = (torch.arange(n_outer_nodes)[:, None] +
                 ind[None]) % n_outer_nodes
...
def uniform_upsample(poly, p_num):
    ...
    edge_num = torch.round(edge_len * p_num /
                           torch.sum(edge_len, dim=2)[..., None]).long()
    ...
    extreme_utils.calculate_edge_num(edge_num, edge_num_sum, edge_idx_sort,
                                     p_num)
    edge_num_sum = torch.sum(edge_num, dim=2)
...
def zoom_poly(poly, scale):
    mean = (poly.min(dim=1, keepdim=True)[0] +
            poly.max(dim=1, keepdim=True)[0]) * 0.5
    poly = poly - mean
