...
def decode_ext_hm(ext_hm, bbox, vote, ct):
    ...
    radius = gaussian_radius(bbox[..., 2] - bbox[..., 0],
                             bbox[..., 3] - bbox[..., 1])
    bbox = torch.round(bbox).long()
    extreme_point = extreme_utils.collect_extreme_point(
        ext_hm, bbox, radius, vote.permute(0, 2, 3, 1), ct)
    return extreme_point


def get_quadrangle(box):
    x_min, y_min, x_max, y_max = box[..., 0], box[..., 1], box[...,
                                                               2], box[..., 3]
    quadrangle = [(x_min + x_max) / 2., y_min, x_min, (y_min + y_max) / 2.,
                  (x_min + x_max) / 2., y_max, x_max, (y_min + y_max) / 2.]
    quadrangle = torch.stack(quadrangle, dim=2).view(x_min.size(0),
                                                     x_min.size(1), 4, 2)
    return quadrangle
...
def get_box(box):
    ...
    box = torch.stack(box, dim=2).view(-1, 8, 2)[None]
    # box = torch.stack(box, dim=2).view(-1, 4, 2)[None]
    return box
...
def get_octagon(ex):
    ...
    octagon = [
        ex[..., 0, 0], ex[..., 0, 1],
        torch.max(ex[..., 0, 0] - w / x, l), ex[..., 0, 1], ex[..., 1, 0],
        torch.max(ex[..., 1, 1] - h / x,
                  t), ex[..., 1, 0], ex[..., 1, 1], ex[..., 1, 0],
        torch.min(ex[..., 1, 1] + h / x, b),
        torch.max(ex[..., 2, 0] - w / x,
                  l), ex[..., 2, 1], ex[..., 2, 0], ex[..., 2, 1],
        torch.min(ex[..., 2, 0] + w / x, r), ex[..., 2, 1], ex[..., 3, 0],
        torch.min(ex[..., 3, 1] + h / x, b), ex[..., 3, 0], ex[..., 3, 1],
        ex[..., 3, 0],
