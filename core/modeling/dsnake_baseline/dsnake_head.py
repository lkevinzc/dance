...
class GraphConvolution(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        forward_input = input[..., adj.view(-1)].view(input.size(0), -1, adj.size(0), adj.size(1)).mean(3)
...
class DsnakeHead(nn.Module):
    ...
    def sample_octagons_fast(self, pred_instances):
        ...
        for im_i in range(len(pred_instances)):
            ...
            poly_sample_locations.append(self.get_octagon(ext_points, self.num_sampling))
    ...
    def get_simple_contour(gt_masks):
        ...
        for polys in polygon_mask:
            ...
            contour = list(map(lambda x: np.array(x).reshape(-1, 2).astype(np.float32), polys))
    ...
    def compute_targets_for_polys(self, targets):
        ...
        for im_i in range(len(targets)):
            ...
            quadrangle = self.get_quadrangle(bboxes).cpu().numpy().reshape(-1, 4, 2)    # (k, 4, 2)

            if self.initial == 'octagon':
                # [t_H_off, l_V_off, b_H_off, r_V_off]
                ext_pts_off = self.get_simple_extreme_points(gt_masks.polygons).to(bboxes.device)
                ...
                # N x 16 (ccw)
                octagons = ExtremePoints(torch.cat([ex_t, ex_l, ex_b, ex_r],
                                                   dim=1)).get_octagons().cpu().numpy().reshape(-1, 8, 2)
            else:
            ...
            # per instance
            for (quad, oct, cnt, ext, w, h) in zip(quadrangle, octagons, contours, ext_points, ws, hs):
                if cnt is None:
                    continue
                ...
                assert not Polygon(cnt).exterior.is_ccw, '1) contour must be clock-wise!'
                ...
                oct_sampled_targets = self.uniform_sample(cnt, len(cnt) * self.num_sampling)  # (big, 2)
                # i) find a single nearest, so that becomes ordered point sets

                tt_idx = np.argmin(np.power(oct_sampled_targets - oct_sampled_pts[0], 2).sum(axis=1))
        ...
        return {"quadrangle_locs": init_sample_locations,
    ...
    def reorder(self, oct_sampled_targets, oct_sampled_pts):
        ...
        enumerated_ind = torch.fmod(ind2 + ind1.view(-1, 1), self.num_sampling).view(-1).long()
        enumerated_targets = oct_sampled_targets[enumerated_ind].reshape(-1, self.num_sampling, 2)
    ...
    def get_locations_feature(self, features, locations, image_idx):
        ...
        sampled_features = torch.zeros([locations.size(0),
                                          features.size(1),
                                          locations.size(1)]).to(locations.device)
        for i in range(batch_size):
            per_im_loc = locations[image_idx == i].unsqueeze(0)
            feature = torch.nn.functional.grid_sample(features[i:i + 1], per_im_loc,
                                                      align_corners=False)[0].permute(1, 0, 2)
    ...
    def init(self, deformer, features, locations, image_idx):
        ...
        if self.conv_type == 'ccn':
            ...
            adj = self.get_adj_ind(self.num_adj, concat_features.size(2), concat_features.device)
    ...
    def forward(self, features, pred_instances=None, targets=None):
        if self.training:
            ...
            locations, reg_targets, scales, image_idx = training_targets["octagon_locs"],\
                                                        training_targets["octagon_targets"], \
                                                        training_targets["scales"], \
                                                        training_targets["image_idx"]
            init_locations, init_targets = training_targets['quadrangle_locs'], \
        ...
        if not self.training:
            ...
            for i, instance_per_im in enumerate(pred_instances):
                ...
                pred_exts_per_im[..., 0] = torch.clamp(pred_exts_per_im[..., 0], min=0, max=w - 1)
                pred_exts_per_im[..., 1] = torch.clamp(pred_exts_per_im[..., 1], min=0, max=h - 1)
                if not instance_per_im.has('ext_points'):
                    instance_per_im.ext_points = ExtremePoints(pred_exts_per_im)
                    poly_sample_locations.append(self.get_octagon(pred_exts_per_im,
                                                              self.num_sampling))
                else:   # NOTE: For GT Input testing
                    # print('Using GT EX')
                    poly_sample_locations.append(self.get_octagon(instance_per_im.ext_points.tensor,
                                                                  self.num_sampling))
        ...
        for i in range(self.num_iter):
            ...
            if i == 0:
                ...
                pred_location = self.evolve(deformer, features, pred_location, image_idx)
            location_preds.append(pred_location)

        if self.training:
            evolve_loss = 0
            for pred in location_preds:
                evolve_loss += self.loss_reg(pred / scales[:, None, None],
                                      reg_targets / scales[:, None, None]) / 3
            ...
            new_instances = self.predict_postprocess(pred_instances, locations, location_preds, image_idx)
            return new_instances, {}

    def predict_postprocess(self,
                            pred_instances,
                            locations,
                            location_preds,
                            image_idx):
