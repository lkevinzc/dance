...
class DiceLoss(nn.Module):
    # TODO: quite un-clean, modify later
    def __init__(self, bce_weight=0, ignore_value=255):
    ...
    def forward(self, pred, target, weight=None):
        ...
        # smooth value = 1, different from epsilon.
        # dice_coef = (2 * pg + 1.) / (p2 + g2 + 1.)
...
def load_model(net,
    ...
    if not os.path.exists(model_dir):
        ...
        print(colored('at ' + model_dir, 'red'))
        return 0
    ...
    print('load model: {}'.format(os.path.join(model_dir,
                                               '{}.pth'.format(pth))))
    pretrained_model = torch.load(os.path.join(model_dir,
                                               '{}.pth'.format(pth)))
...
def load_network(net, model_dir, resume=True, epoch=-1, strict=True):
    ...
    pths = [
        int(pth.split('.')[0]) for pth in os.listdir(model_dir) if 'pth' in pth
    ]
    if len(pths) == 0:
        print(colored('WARNING: NO MODEL LOADED !!!', 'red'))
        print(colored('at ' + model_dir, 'red'))
...
def load_specific_network(net, model_dir, resume=True, epoch=-1, strict=True):
    if not resume:
        return 0
