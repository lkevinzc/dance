...
class Dataset(data.Dataset):
    def __init__(self, ann_file, data_root, split):
        ...
        self.anns = np.array([
            img_id for img_id in self.anns
    ...
    def read_original_data(self, anno, path):
        ...
        ] for obj in anno if not isinstance(obj['segmentation'], dict)]
        cls_ids = [
    ...
    def transform_original_data(self, instance_polys, flipped, width,
        ...
        for instance in instance_polys:
            ...
            polys = snake_cityscapes_utils.transform_polys(
    ...
    def get_extreme_points(self, instance_polys):
        ...
        for instance in instance_polys:
            points = [
                snake_cityscapes_utils.get_extreme_points(poly)
    ...
    def prepare_evolution(self, poly, extreme_point, img_init_polys,
        ...
        img_gt_poly = snake_cityscapes_utils.uniformsample(
            poly, spline_poly_num)
        tt_idx = np.argmin(
            np.power(img_gt_poly - img_init_poly[0], 2).sum(axis=1))
        img_gt_poly = np.roll(img_gt_poly, -tt_idx,
                              axis=0)[::snake_config.spline_num]
        can_gt_poly = snake_cityscapes_utils.img_poly_to_can_poly(
            img_gt_poly, x_min, y_min, x_max, y_max)
    ...
    def __getitem__(self, index):
        ...
        # print(path_join(self.edge_map_root, path.split('/')[-1]))
        edge_map = cv2.imread(
