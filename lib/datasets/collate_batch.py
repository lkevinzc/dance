...
def snake_collator(batch):
    ...
    # init
    i_it_4pys = torch.zeros(
        [batch_size, ct_num, snake_config.init_poly_num, 2], dtype=torch.float)
    c_it_4pys = torch.zeros(
        [batch_size, ct_num, snake_config.init_poly_num, 2], dtype=torch.float)
    ...
    # evolution
    i_it_pys = torch.zeros([batch_size, ct_num, snake_config.poly_num, 2],
                           dtype=torch.float)
    c_it_pys = torch.zeros([batch_size, ct_num, snake_config.poly_num, 2],
                           dtype=torch.float)
    i_gt_pys = torch.zeros([batch_size, ct_num, snake_config.gt_poly_num, 2],
                           dtype=torch.float)
    c_gt_pys = torch.zeros([batch_size, ct_num, snake_config.gt_poly_num, 2],
                           dtype=torch.float)
    if ct_num != 0:
...
def rcnn_snake_collator(batch):
    ...
    if act_num != 0:
        awh[act_01] = torch.Tensor(sum([b['awh'] for b in batch], []))
        act_ind[act_01] = torch.LongTensor(
            sum([b['act_ind'] for b in batch], []))
    ...
    if cp_num != 0:
        cp_wh[cp_01] = torch.Tensor(
            sum(sum([b['cp_wh'] for b in batch], []), []))
        cp_ind[cp_01] = torch.LongTensor(
            sum(sum([b['cp_ind'] for b in batch], []), []))

    cp_hm_ = torch.zeros(
        [batch_size, act_num, 1, snake_config.cp_h, snake_config.cp_w],
        dtype=torch.float)
...
def dance_collator(batch):
    ...
    # dance evolution
    init_box = torch.zeros([batch_size, ct_num, snake_config.poly_num, 2],
                           dtype=torch.float)
    # c_it_pys = torch.zeros([batch_size, ct_num, snake_config.poly_num, 2], dtype=torch.float)
    targ_poly = torch.zeros([batch_size, ct_num, snake_config.gt_poly_num, 2],
                            dtype=torch.float)
    ...
    if ct_num != 0:
        ...
        # c_it_pys[ct_01] = torch.Tensor(sum([b['c_it_py'] for b in batch], []))
        targ_poly[ct_01] = torch.Tensor(
            sum([b['targ_poly'] for b in batch], []))
        # c_gt_pys[ct_01] = torch.Tensor(sum([b['c_gt_py'] for b in batch], []))
        whs[ct_01] = torch.Tensor(sum([b['whs'] for b in batch], []))

    evolution = {'init_box': init_box, 'targ_poly': targ_poly, 'whs': whs}
    ret.update(evolution)

    # edge detection
    edge_map = default_collate([b['edge'] for b in batch])
    edge = {'edge_map': edge_map}
    ret.update(edge)

    return ret


def rcnn_dance_collator(batch):
    ret = {'inp': default_collate([b['inp'] for b in batch])}
